    ...
    return (
      <main>
        ...
        <div className="text-center py-32 text-stone-500">
          <p className="text-2xl font-bold">{tr("productNotFound")}</p>
          <Link href="/shop" className="mt-4 inline-block text-brown underline">
            {tr("backToShop")}
          </Link>
        </div>
        <Footer />
    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-stone-400 mb-8">
          <Link href="/" className="hover:text-brown cursor-pointer transition-colors">{tr("home")}</Link>
          <span>/</span>
          <Link href="/shop" className="hover:text-brown cursor-pointer transition-colors">{tr("shop")}</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
          ...
          <div className="relative">
            ...
            <div className="absolute top-4 end-4 flex flex-col gap-2">
              ...
              {discount > 0 && (
                <span className="bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-lg shadow">-{discount}%</span>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? "text-gold" : "text-stone-200"} fill-current`} viewBox="0 0 20 20">
            ...
            <div className="flex items-baseline gap-3 mb-5">
              ...
              {product.originalPrice && (
                <span className="text-stone-400 text-lg line-through">{product.originalPrice} {tr("aed")}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex items-center border border-stone-200 rounded-xl overflow-hidden">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-10 h-10 flex items-center justify-center text-stone-600 hover:bg-stone-50 transition-colors cursor-pointer">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" /></svg>
                </button>
                <span className="w-10 text-center font-bold text-sm">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="w-10 h-10 flex items-center justify-center text-stone-600 hover:bg-stone-50 transition-colors cursor-pointer">
            ...
            <div className="flex gap-3">
              ...
              >
                {added ? (
                  ...
                  <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>{tr("addToCartBtn")}</>
                )}
              </button>
              <button
                onClick={() => toggleWishlist(product.id)}
                className={`w-12 h-12 flex items-center justify-center rounded-xl border-2 transition-all active:scale-95 cursor-pointer ${isWishlisted ? "border-red-400 bg-red-50 text-red-500" : "border-stone-200 text-stone-400 hover:border-red-300 hover:text-red-400"}`}
                aria-label="wishlist"
              >
