    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-stone-400 mb-8">
          <Link href="/" className="hover:text-brown cursor-pointer transition-colors">الرئيسية</Link>
          <span>/</span>
          <Link href="/shop" className="hover:text-brown cursor-pointer transition-colors">المتجر</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-stone-500 text-sm font-medium">{product.rating} ({product.reviews} تقييم)</span>
            </div>

            {/* Price */}
            <div className="flex items-baseline gap-3 mb-5">
              <span className="text-3xl font-black text-brown">{product.price}</span>
              <span className="text-base font-semibold text-brown">درهم</span>
              {product.originalPrice && (
                <span className="text-stone-400 text-lg line-through">{product.originalPrice} درهم</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mb-5">
              <span className="text-sm font-semibold text-stone-700">الكمية:</span>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm transition-all duration-200 active:scale-95 cursor-pointer ${
                  added ? "bg-green-500 text-white" : "bg-brown hover:bg-brown-dark text-white"
                }`}
              >
                {added ? (
                  <>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {[
                { icon: "🚚", text: "توصيل مجاني داخل الإمارات" },
        ...
        {related.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-ink mb-6">منتجات مشابهة</h2>
