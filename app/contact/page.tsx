    ...
    <main className="min-h-screen" dir={dir}>
      ...
      <div className="bg-gradient-to-l from-brown to-brown-dark text-white py-12 px-4">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-black mb-1">{tr("contactTitle")}</h1>
          <p className="text-white/70 text-sm">{tr("contactSub")}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-5">
            <h2 className="text-xl font-bold text-ink mb-4">{tr("contactInfo")}</h2>
            ...
            {contactItems.map((item) => (
              <div key={item.label} className="flex items-start gap-4 bg-white rounded-2xl p-4 border border-stone-100 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-ink" dir={item.valueDir}>{item.value}</p>
                </div>
              </div>
            ))}
            ...
            >
              <svg className="w-5 h-5 fill-current flex-none" viewBox="0 0 24 24">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 md:p-8">
              <h2 className="text-xl font-bold text-ink mb-6">{tr("sendMessage")}</h2>
              ...
              {sent ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-bold text-ink mb-2">{tr("sentTitle")}</h3>
                  <p className="text-stone-500 text-sm">{tr("sentSub")}</p>
                  <button
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-1.5">
                      {tr("emailLabel")} <span className="text-red-400">*</span>
                    </label>
                    <input
                      type="email"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-1.5">
                      {tr("subjectLabel")} <span className="text-red-400">*</span>
                    </label>
                    <select
                      required
                      value={form.subject}
                    ...
                    >
                      ...
                      <option value="order">{tr("subjectOrder")}</option>
                      <option value="product">{tr("subjectProduct")}</option>
                      <option value="wholesale">{tr("subjectWholesale")}</option>
                      <option value="return">{tr("subjectReturn")}</option>
                      <option value="other">{tr("subjectOther")}</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-1.5">
                      {tr("messageLabel")} <span className="text-red-400">*</span>
                    </label>
                    <textarea
                      required
                      rows={5}
